...
main: loop {
    ...
    for @enemies {
        SDL_RenderCopy($render, $enemy_texture, SDL_Rect, SDL_Rect.new(:x(.pos.re.Int - 32), :y(.pos.im.Int - 32), :w(64), :h(64)));
    }

    SDL_SetRenderDrawColor($render, 0, 0, 255, 200);
    for @bullets {
        SDL_RenderDrawLine($render, .pos.re.Int, .pos.im.Int, (.pos.re - .vel.re * 0.05).Int, (.pos.im - .vel.im * 0.05).Int);
    }
